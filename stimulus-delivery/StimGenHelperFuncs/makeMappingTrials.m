function [trials] = makeMappingTrials(table)
...
N_trials = constants.N_Grid_Locations_x * constants.N_Grid_Locations_y;
...
for r = 1:constants.Repeats
    p = randperm(N_trials);
    stim_locations = [stim_locations p];
end
...
for i = 1:N_trials*constants.Repeats
    trials(i).stimType = "Receptive Field Mapping";
    for j = 1:numel(fieldname)
        trials(i).(fieldname{j}) = constants.(fieldname{j});
    end
    trials(i).stim_location = stim_locations(i);
end
end